...
For other locations, we need to consider using new scenic spot data for constraints. Here we define:
$$A=V-kC$$
Here, A represents the attractiveness of the scenic spot, V represents the inherent value of the scenic spot, C still denotes the number of tourists, and k is a constant that describes the negative impact of the number of tourists on the scenic spot. With this formula, as long as the corresponding data of the scenic spot is obtained for calculation, the optimal number of people can be obtained.

Not only that, for scenic spots with fewer people, we can attract more tourists by reducing the tourism tax, and this model is also applicable. For scenic spots with a large number of people, we suggest using both increasing taxes and limiting the number of tourists to reduce the number of tourists, which can achieve a dynamic balance.
